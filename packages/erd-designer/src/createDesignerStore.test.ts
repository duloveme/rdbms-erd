...
describe("createDesignerStore + zundo", () => {
    const createAcmeAdapter = (): DbMetaAdapter => {
        ...
        return {
            ...
            getDialectMeta: (dialect) =>
                dialect === "acme" ? acmeMeta : base.getDialectMeta(dialect),
            getDefaultPhysicalType: (dialect, logicalType) =>
                dialect === "acme"
                    ? acmeTypes[logicalType]
                    : base.getDefaultPhysicalType(dialect, logicalType),
    ...
    it("setColumnLogicalName syncs physical name when it still matched previous logical name", () => {
        ...
        expect(
            useStore.getState().doc.model.tables[0].columns[0].physicalName,
        ).toBe("이름");
    ...
    it("alignSelected is one undo step", () => {
        ...
        s.addTable(
            { id: "a", logicalName: "A", physicalName: "TA", columns: [] },
            0,
            0,
        );
        s.addTable(
            { id: "b", logicalName: "B", physicalName: "TB", columns: [] },
            100,
            0,
        );
    ...
    it("removeRelationship deletes auto-created FK column", () => {
        ...
        useStore
            ...
            .setTableColumns("fk", [
                ...useStore
                    .getState()
                    .doc.model.tables.find((t) => t.id === "fk")!.columns,
                createColumn("postgres", {
                    id: "fkcol",
                    logicalName: "ID",
                    logicalType: "NUMBER",
        ...
        const fkTable = useStore
            .getState()
            .doc.model.tables.find((t) => t.id === "fk")!;
    ...
    it("setRelationshipCanvasLineHidden toggles relationship.canvasLineHidden", () => {
        ...
        expect(
            useStore.getState().doc.model.relationships[0].canvasLineHidden,
        ).toBe(true);
    ...
    it("setRelationshipLinePivotRatio stores clamped ratio", () => {
        ...
        expect(
            useStore.getState().doc.model.relationships[0].linePivotRatio,
        ).toBe(1);
    ...
    it("setRelationshipSourceLineRatio stores clamped ratio", () => {
        ...
        expect(
            useStore.getState().doc.model.relationships[0].sourceLineRatio,
        ).toBe(1);
    ...
    it("setRelationshipSourceLineY stores absolute y and supports undo/redo", () => {
        ...
        expect(useStore.getState().doc.model.relationships[0].sourceLineY).toBe(
            142.5,
        );
        useStore.temporal.getState().undo();
        expect(
            useStore.getState().doc.model.relationships[0].sourceLineY,
        ).toBeUndefined();
        useStore.temporal.getState().redo();
        expect(useStore.getState().doc.model.relationships[0].sourceLineY).toBe(
            142.5,
    ...
    it("deleteSelection removes table and connected relationships in one undo step", () => {
        ...
        expect(useStore.getState().doc.model.tables.map((t) => t.id)).toEqual([
            "b",
        ]);
        ...
        expect(
            useStore.getState().doc.model.relationships.map((r) => r.id),
        ).toEqual(["r-ab"]);
    });
    ...
    it("deleteSelection removes relationship and FK column with undo/redo", () => {
        ...
        const fkTableAfterDelete = useStore
            .getState()
            .doc.model.tables.find((t) => t.id === "fk")!;
        expect(useStore.getState().doc.model.relationships).toHaveLength(0);
        expect(fkTableAfterDelete.columns.some((c) => c.id === "fkcol")).toBe(
            false,
        );

        useStore.temporal.getState().undo();
        const fkTableAfterUndo = useStore
            .getState()
            .doc.model.tables.find((t) => t.id === "fk")!;
        expect(
            useStore.getState().doc.model.relationships.map((r) => r.id),
        ).toEqual(["r1"]);
        expect(fkTableAfterUndo.columns.some((c) => c.id === "fkcol")).toBe(
            true,
        );

        useStore.temporal.getState().redo();
        const fkTableAfterRedo = useStore
            .getState()
            .doc.model.tables.find((t) => t.id === "fk")!;
        expect(useStore.getState().doc.model.relationships).toHaveLength(0);
        expect(fkTableAfterRedo.columns.some((c) => c.id === "fkcol")).toBe(
            false,
        );
    });
    ...
    it("setColumnLogicalType uses host adapter default physical type", () => {
        ...
        const useStore = createDesignerStore({
            initialDialect: "acme",
            coreOptions: { dbMetaAdapter: adapter },
        });
        useStore.getState().addTable(
            {
                id: "t1",
                ...
                columns: [
                    createColumn(
                        "acme",
                        { id: "c1", logicalName: "컬럼", logicalType: "TEXT" },
        ...
        expect(
            useStore.getState().doc.model.tables[0].columns[0].physicalType,
        ).toBe("BOOL");
    });
});
