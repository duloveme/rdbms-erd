...
export interface DesignerState {
  ...
  updateTable: (tableId: string, updater: (table: TableModel) => void) => void;
  setColumnLogicalType: (tableId: string, columnId: string, logicalType: LogicalDataType) => void;
  setColumnPhysicalType: (tableId: string, columnId: string, physicalType: string) => void;
  /** 논리 이름 변경. 물리 이름이 이전 논리 이름과 같았다면 새 논리 이름으로 동기화한다. */
  setColumnLogicalName: (tableId: string, columnId: string, logicalName: string) => void;
  /** 컬럼 물리 이름(식별자). DDL·노드 표시에 사용된다. */
  setColumnPhysicalName: (tableId: string, columnId: string, physicalName: string) => void;
  ...
  return create<DesignerState>()(
    temporal(
      immer((set) => ({
        ...
        removeTable: (tableId) =>
          set((state) => {
            state.doc.model.tables = state.doc.model.tables.filter((t) => t.id !== tableId);
            state.doc.model.relationships = state.doc.model.relationships.filter(
              (r) => r.sourceTableId !== tableId && r.targetTableId !== tableId
            );
            state.doc.model.indexes = state.doc.model.indexes.filter((i) => i.tableId !== tableId);
          ...
          set((state) => {
            ...
            if (rel?.targetColumnId) {
              ...
              if (targetTable) {
                ...
                const usedByOthers = state.doc.model.relationships.some(
                  (r) =>
                    r.id !== relationshipId &&
            ...
            state.doc.model.relationships = state.doc.model.relationships.filter((r) => r.id !== relationshipId);
          }),
        updateTable: (tableId, updater) =>
          set((state) => {
            const table = state.doc.model.tables.find((item) => item.id === tableId);
            if (!table) return;
            updater(table);
          }),
        setColumnLogicalType: (tableId, columnId, logicalType) =>
          set((state) => {
            const table = state.doc.model.tables.find((t) => t.id === tableId);
            const column = table?.columns.find((c) => c.id === columnId);
            if (!column) return;
            column.logicalType = logicalType;
            column.physicalType = defaultPhysicalType(state.doc.model.dialect, logicalType, options.coreOptions);
          }),
        ...
        setNodePositions: (positions) =>
          set((state) => {
            for (const [tableId, pos] of Object.entries(positions)) {
              state.doc.layout.nodePositions[tableId] = { x: pos.x, y: pos.y };
            }
          }),
        alignSelected: (tableIds, type) =>
          set((state) => {
            const updates = alignNodePositions(tableIds, state.doc.layout.nodePositions, type);
        ...
        setTableMeta: (tableId, meta) =>
          set((state) => {
            const table = state.doc.model.tables.find((t) => t.id === tableId);
            if (!table) return;
            if (meta.logicalName !== undefined) table.logicalName = meta.logicalName;
            if (meta.physicalName !== undefined) table.physicalName = meta.physicalName;
            if (meta.schemaName !== undefined) {
              if (meta.schemaName === null || !meta.schemaName.trim()) delete table.schemaName;
        ...
        setTableColumns: (tableId, columns) =>
          set((state) => {
            const table = state.doc.model.tables.find((t) => t.id === tableId);
            if (!table) return;
            const prevColumnsById = new Map(table.columns.map((c) => [c.id, c]));
            const colIds = new Set(columns.map((c) => c.id));
            state.doc.model.relationships = state.doc.model.relationships.filter((r) => {
              if (r.sourceTableId === tableId && r.sourceColumnId && !colIds.has(r.sourceColumnId)) {
                return false;
              }
              if (r.targetTableId === tableId && r.targetColumnId && !colIds.has(r.targetColumnId)) {
                return false;
              }
              return true;
            });
            ...
            for (const rel of state.doc.model.relationships) {
              if (rel.sourceTableId !== tableId || !rel.sourceColumnId || !rel.targetColumnId) continue;
              const sourceCol = table.columns.find((c) => c.id === rel.sourceColumnId);
              if (!sourceCol || !sourceCol.isPrimaryKey) continue;
              const prevSource = prevColumnsById.get(rel.sourceColumnId);
              if (!prevSource) continue;
              const typeChanged = prevSource.logicalType !== sourceCol.logicalType || prevSource.physicalType !== sourceCol.physicalType;
