...
import { ChevronDown, ChevronUp, Eraser, Link2, Trash2, Unlink, X } from "lucide-react";
...
function createBlankColumn(dialect: RdbmsDialect, coreOptions?: CoreDbMetaOptions): ColumnModel {
  return createColumn(dialect, {
...
function ensureTrailingBlankColumn(draft: TableModel, dialect: RdbmsDialect, coreOptions?: CoreDbMetaOptions): TableModel {
  const cols = [...draft.columns];
  while (cols.length >= 2 && columnNamesBlank(cols[cols.length - 1]!) && columnNamesBlank(cols[cols.length - 2]!)) {
    cols.pop();
  }
...
export function TableEditDialog({
  ...
  useEffect(() => {
    if (open && table) {
      setDraft(ensureTrailingBlankColumn(cloneTable(table), dialect, coreOptions));
    }
  }, [coreOptions, open, table, dialect]);

  const title = useMemo(() => {
    if (!draft) return t("dialog.tableEdit.fallbackTitle");
    const name = displayMode === "logical" ? draft.logicalName : draft.physicalName;
  ...
  const updateColumn = (index: number, patch: Partial<ColumnModel>) => {
    setDraft((d) => {
      ...
      return ensureTrailingBlankColumn({ ...d, columns: cols }, dialect, coreOptions);
    });
  };
  ...
  const removeColumn = (index: number) => {
    setDraft((d) => {
      ...
      return ensureTrailingBlankColumn({ ...d, columns: d.columns.filter((_, i) => i !== index) }, dialect, coreOptions);
    });
  };

  const nameCaption = t("dialog.tableName");
  const getColumnName = (index: number) =>
    displayMode === "logical" ? draft.columns[index].logicalName : draft.columns[index].physicalName;
  ...
  const gridClass = displayMode === "physical" ? "erd-dialog-col-grid erd-dialog-col-grid--physical" : "erd-dialog-col-grid erd-dialog-col-grid--logical";
  const supportsSchema = dialectSupportsSchema(dialect, coreOptions);
  const logicalTypes = resolveDialectMetas(coreOptions).find((m) => m.id === dialect)?.logicalTypes.map((lt) => lt.id) ?? LOGICAL_DATA_TYPES;

  return (
    <div className="erd-dialog-backdrop" role="presentation">
      <div className="erd-dialog" role="dialog" aria-modal="true" aria-labelledby="erd-table-dialog-title">
        <div className="erd-dialog-header">
          <span id="erd-table-dialog-title">{title}</span>
          <button type="button" className="erd-node-header-btn" aria-label={t("dialog.close")} onClick={onClose}>
            <X size={18} />
          </button>
        </div>
        <div className="erd-dialog-body">
          <div className="erd-field">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 8, alignItems: "center" }}>
              <input
                id="erd-t-name"
                className="erd-input"
                value={displayMode === "logical" ? draft.logicalName : draft.physicalName}
                onChange={(e) => {
                  const nextName = e.target.value;
                  if (displayMode === "logical") {
                    const next = { ...draft, logicalName: nextName };
                    if (draft.physicalName === draft.logicalName) {
                      next.physicalName = nextName;
                    }
                    setDraft(next);
                  } else {
                    setDraft({ ...draft, physicalName: nextName });
                  }
                }}
              />
              <input
                ...
                style={{ width: 42, height: 36, border: "1px solid var(--erd-border)", borderRadius: 8, padding: 2 }}
              />
              <button
                type="button"
          ...
          {displayMode === "physical" && supportsSchema ? (
            <div className="erd-field">
              <label htmlFor="erd-t-schema">{t("dialog.tableSchema")}</label>
              <input
                id="erd-t-schema"
                className="erd-input"
          ...
          <div className={`${gridClass} erd-dialog-col-grid--header`}>
            <span className="erd-dialog-col-head-text">{t("dialog.column.name")}</span>
            <span className="erd-dialog-col-head-text">{t("dialog.column.type")}</span>
            {displayMode === "physical" ? <span className="erd-dialog-col-head-text">{t("dialog.column.default")}</span> : null}
            <span className="erd-dialog-col-head-tight" title={t("dialog.column.pkTitle")}>
              {t("dialog.column.pk")}
            </span>
            <span className="erd-dialog-col-head-tight" title={t("dialog.column.nullTitle")}>
              {t("dialog.column.nullShort")}
            </span>
            <span className="erd-dialog-col-head-spacer" aria-hidden="true" />
            <span className="erd-dialog-col-head-spacer" aria-hidden="true" />
            <span className="erd-dialog-col-head-spacer" aria-hidden="true" />
            <span className="erd-dialog-col-head-spacer" aria-hidden="true" />
          </div>
          {draft.columns.map((col, index) => (
            <div
              ...
              style={{
                ...
                borderRadius: index === 0 ? "10px 10px 0 0" : index === draft.columns.length - 1 ? "0 0 10px 10px" : 0,
                padding: "6px 8px",
                marginBottom: 0,
            ...
            >
              <div className={gridClass}>
                <input
                  ...
                  style={{ fontStyle: col.isForeignKey ? "italic" : "normal", minWidth: 0 }}
                />
                {displayMode === "logical" ? (
                  <select
                    ...
                    onChange={(e) => {
                      const lt = e.target.value as LogicalDataType;
                      updateColumn(index, {
                        logicalType: lt,
                        physicalType: defaultPhysicalType(dialect, lt, coreOptions)
                      });
                    }}
                  >
                    {logicalTypes.map((t) => (
                  ...
                  <input
                    ...
                    onChange={(e) => updateColumn(index, { physicalType: e.target.value.toUpperCase() })}
                    style={{ minWidth: 0 }}
                  />
                )}
                ...
                <div style={{ display: "flex", gap: 2, justifyContent: "center" }}>
                  ...
                  <button
                    type="button"
                    className="erd-toolbar-btn"
                    style={{ minWidth: 28, padding: 0, color: "var(--erd-danger)" }}
                ...
                <button
                  ...
                  style={{ visibility: col.isForeignKey ? "visible" : "hidden" }}
                  disabled={!col.isForeignKey}
                  title={col.showFkRelationLine !== false ? t("dialog.column.fkLineOn") : t("dialog.column.fkLineOff")}
                  aria-label={col.showFkRelationLine !== false ? t("dialog.column.fkLineOn") : t("dialog.column.fkLineOff")}
                  aria-pressed={col.isForeignKey ? col.showFkRelationLine !== false : undefined}
                  onClick={() => {
                    if (!col.isForeignKey) return;
                    updateColumn(index, { showFkRelationLine: !(col.showFkRelationLine !== false) });
                  }}
                >
                  {col.showFkRelationLine !== false ? <Link2 size={16} /> : <Unlink size={16} />}
                </button>
                <input
                  type="color"
                ...
                <button
                  ...
                  onClick={() => updateColumn(index, { color: undefined })}
                >
                  <Eraser size={16} />
                </button>
              </div>
        ...
        <div className="erd-dialog-footer">
          <button type="button" className="erd-btn erd-btn--ghost" onClick={onClose}>
            {t("dialog.cancel")}
          </button>
          <button
            type="button"
            ...
            onClick={() => {
              onSave(normalizeTableForSave(draft, dialect, t, coreOptions));
              onClose();
            }}
          >
            {t("dialog.save")}
